...
async function getArticles() {
  try {
    // 1️  Récupérer le JSON
    const res = await fetch("RGL_L1.json");
    const results = await res.json();

    // 2️   Trier la liste par ordre alphabétique
    datarray = orderList(results);

    // 3️   Créer et afficher la liste dans le DOM
